...
VOCABULARY: List[str] = [
    "medicine", "law", "software", "finance", "art", "sports", "cooking", "environment", "philosophy", "education"
]
...
def active_labels_from_verify_x(x_wm: Sequence[int], modulus: int) -> List[str]:
    """
    Vocabulary labels NLI marks **active** on the transcript used to build ``x_wm``.

    Matches ``attr_x_nli.derive_x``: prefix entries are absence bits reduced mod ``modulus``;
    active label ⇒ coordinate ``≡ 0 (mod modulus)``.
    """
    out: List[str] = []
...
def pick_unrelated_keyword_for_policy(
    ...
    accidental ``detect=True`` when NLI already treats that label as active on the transcript.
    """
