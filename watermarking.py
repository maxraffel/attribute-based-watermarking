...
from closed_vocab import VOCABULARY, derive_x, issue_constrained_key_for_keywords
...
def setup(modulus: int) -> cprf.MasterKey:
    return cprf.keygen(modulus, len(VOCABULARY))
...
def issue_unconstrained(sk: cprf.MasterKey) -> cprf.ConstrainedKey:
    ...
    return issue(sk, [0] * len(VOCABULARY))


def issue_keyword_policy(sk: cprf.MasterKey, required: List[str]) -> cprf.ConstrainedKey:
    """Constrained key that matches master eval iff every known required keyword appears in the text."""
...
def detect(dk: cprf.ConstrainedKey, prompt: str, watermarked_text: str) -> bool:
    """Recover PRC bits from ``watermarked_text``; x is always derived from greedy ``prompt`` baseline."""
